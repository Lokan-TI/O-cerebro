...
- O universo de **Faturamento NF** foi centralizado em `base44/shared/invoiceUniverse.ts`: saída (`fl_ent_sai='S'`), `fl_can_nf` diferente de `S/1`, `dt_cancelamento IS NULL`, `dt_anul_nf IS NULL`, data `nf.dt_emi_nf`. Refresh, clientes ativos, Cliente 360, conversão, lifecycle, projeções, Metric Registry, reconciliação e Cérebro IA devem reutilizar esse contrato e não reescrever flags localmente.
- A tela de clientes distingue `faturamento_fiscal_total` de `faturamento_atribuido_clientes`; NFs válidas sem `cd_pessoa` são reportadas em `faturamento_sem_cliente` e não podem desaparecer silenciosamente de uma comparação.
- `refreshErpData` deve usar a janela explícita recebida do filtro global para os KPIs principais do snapshot. Quando `snapshot.analytics_period` coincide com o período aplicado, `periodScope` usa o total fiscal extraído diretamente da `nf`, evitando reconstrução aproximada por meses.
