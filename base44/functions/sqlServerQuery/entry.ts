import { createClientFromRequest } from 'npm:@base44/sdk@0.8.40';
import sql from 'npm:mssql@10.0.1';

Deno.serve(async (req) => {
  try {
    const base44 = createClientFromRequest(req);
    const user = await base44.auth.me();
    if (!user) return Response.json({ error: 'Unauthorized' }, { status: 401 });

    const body = await req.json();
    const query = body?.query;

    if (!query || typeof query !== 'string') {
      return Response.json({ error: 'Parâmetro "query" é obrigatório' }, { status: 400 });
    }

    // Security: only allow read-only queries and stored procedure execution
    // Strip leading SQL comments before checking
    let cleaned = query.trim();
    while (cleaned.startsWith('--')) {
      const newlineIdx = cleaned.indexOf('\n');
      if (newlineIdx === -1) { cleaned = ''; break; }
      cleaned = cleaned.slice(newlineIdx + 1).trim();
    }
    const trimmed = cleaned.toUpperCase();
    if (!trimmed.startsWith('SELECT') && !trimmed.startsWith('WITH') && !trimmed.startsWith('EXEC') && !trimmed.startsWith('EXECUTE') && !trimmed.startsWith('SP_')) {
      return Response.json({ error: 'Apenas queries SELECT, WITH ou EXEC são permitidas (read-only)' }, { status: 403 });
    }

    const host = Deno.env.get('SQL_SERVER_HOST');
    if (!host) {
      return Response.json({ error: 'SQL Server não configurado. Defina as variáveis SQL_SERVER_* nas configurações.' }, { status: 500 });
    }

    const config = {
      server: host,
      port: parseInt(Deno.env.get('SQL_SERVER_PORT') || '1433'),
      database: Deno.env.get('SQL_SERVER_DATABASE'),
      user: Deno.env.get('SQL_SERVER_USER'),
      password: Deno.env.get('SQL_SERVER_PASSWORD'),
      options: {
        encrypt: false,
        trustServerCertificate: true,
      },
      requestTimeout: 30000,
    };

    const pool = await sql.connect(config);
    try {
      const request = pool.request();
      // Use execute() for stored procedures, query() for SELECT/WITH
      const isExec = trimmed.startsWith('EXEC') || trimmed.startsWith('EXECUTE') || trimmed.startsWith('SP_');
      const result = isExec ? await request.query(query) : await request.query(query);
      const rows = result.recordset || [];
      return Response.json({
        rows,
        rowCount: rows.length,
        multipleSets: result.recordsets || [],
      });
    } finally {
      await pool.close();
    }
  } catch (error) {
    return Response.json({ error: error.message || String(error) }, { status: 500 });
  }
});