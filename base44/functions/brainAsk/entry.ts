...
async function buildSchemaBrief(base44: any): Promise<string> {
  try {
    ...
    if (!rows || rows.length === 0) return FALLBACK_SCHEMA;
    ...
    if (plan?.needs_query && plan?.sql) {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        try {
          ...
          if (e instanceof SqlGuardError || attempt === 1) {
          ...
          const fix = await base44.asServiceRole.integrations.Core.InvokeLLM({
            prompt: `A consulta T-SQL abaixo falhou. Corrija-a mantendo as mesmas regras (SELECT único, TOP 200, datas sargáveis, sem comentários/variáveis). Use apenas o dicionário:
