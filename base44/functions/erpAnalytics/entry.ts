...
Deno.serve(async (req) => {
  try {
    ...
    try {
      // Try with cd_empresa column first; fallback to cd_conta only
      let capSql;
      try {
        capSql = `SELECT
          cd_empresa,
    ...
    try {
      ...
      // Batch fetch names
      const codes = [...new Set(topRows.map(r => Number(r.cd_pessoa_cli)))].filter(Boolean);
      const nameMap = {};
      ...
      out.top_clients_car = topRows.map(r => ({
