...
export default function ClientePatrimoniosPanel({ cdPessoa }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const payload = { cd_pessoa: Number(cdPessoa) };
    ...
    return () => { alive = false; };
  }, [cdPessoa, selectedSource]);

  const t = data?.totals || {};

  if (loading) return <div className="text-gray-500 py-8 text-center">Consultando patrimônios no ERP…</div>;
  if (error) return <div className="bg-red-950/40 border border-red-800/50 rounded-lg px-4 py-2 text-red-300 text-sm">{error}</div>;
  if (!data) return null;
