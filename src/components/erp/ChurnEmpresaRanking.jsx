...
// Comparativo legado de filiais pela janela móvel de 12 meses baseada em NF.
// Não representa o churn oficial de locação v3.
    ...
    return (
      <div className="bg-amber-950/30 border border-amber-800/40 rounded-xl px-4 py-3 flex items-center gap-2">
        ...
        <span className="text-amber-300 text-xs">
          Comparativo por NF ainda não calculado neste snapshot. Clique em "Atualizar dados" para gerar a leitura por filial.
        </span>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="rounded-xl border border-red-700/40 bg-red-950/30 p-4">
          <div className="flex items-center gap-2 mb-1"><AlertTriangle className="w-4 h-4 text-red-400" /><span className="text-xs text-gray-400 uppercase">Maior ausência de NF</span></div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-gray-500 text-xs uppercase border-b border-gray-800">
              ...
              <th className="text-right py-2 px-3">Receita da base sem NF</th>
      ...
      <p className="text-xs text-gray-600">
        Comparativo legado: base = clientes com NF entre 24 e 12 meses atrás na filial; ausência = quem não emitiu NF nos últimos 12 meses.
        Esta tabela não verifica fichas de locação abertas e não deve ser interpretada como churn oficial. O consolidado do grupo remove
        duplicidade de clientes atendidos por mais de uma filial. Para churn oficial, use a aba Retenção & Churn (v3).
        Filiais marcadas como "filial nova" não tinham base comparável na janela anterior.
      </p>
    </div>
  );
}