...
import { RESUMO, FUNIL, RETIDO_POR_MES, CLIENTES_WON, RECOMPRAS } from "@/components/google/googleData.jsx";
import {
    ...
    <div className="bg-gray-900 border border-blue-700 rounded-xl p-4 shadow-2xl min-w-[200px]">
      ...
      <div className="space-y-1 text-xs">
        <div className="flex justify-between gap-4">
          ...
          <span className="text-blue-400 font-semibold">{fmtR(data?.receita_novos)}</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-gray-400">Recompra</span>
          <span className="text-purple-400 font-semibold">{fmtR(data?.recompra)}</span>
        </div>
        <div className="border-t border-gray-700 pt-1 mt-1 flex justify-between gap-4">
          <span className="text-gray-300 font-medium">Total</span>
          <span className="text-white font-bold">{fmtR(data?.total)}</span>
        </div>
        {data?.roas_mes != null && (
          <div className="flex justify-between gap-4">
            <span className="text-gray-400">ROAS</span>
            <span className="text-yellow-400 font-semibold">{data.roas_mes.toFixed(2)}x</span>
          </div>
        )}
        {data?.investimento > 0 && (
          <div className="flex justify-between gap-4">
            <span className="text-gray-400">Investimento</span>
            <span className="text-gray-300">{fmtR(data.investimento)}</span>
          </div>
        )}
      </div>
    </div>
...
export default function TabGoogleOverview() {
  ...
  // Recompras do mês selecionado (aproximação pelo índice de RETIDO_POR_MES)
  const recomprasMes = selectedMes
    ? RECOMPRAS.filter(r => {
        const mesStr = selectedMes.replace("/", "-20").replace("Jan", "01").replace("Fev", "02")
          .replace("Mar", "03").replace("Abr", "04").replace("Mai", "05").replace("Jun", "06")
          .replace("Jul", "07").replace("Ago", "08").replace("Set", "09").replace("Out", "10")
          .replace("Nov", "11").replace("Dez", "12");
        // extrai YYYY-MM do selectedMes
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
          ...
          <div className="flex items-center gap-3 text-xs text-gray-400 shrink-0">
            ...
            {selectedMes && (
              <button onClick={() => setSelectedMes(null)} className="ml-2 px-2 py-1 bg-gray-800 rounded text-gray-300 hover:text-white transition-colors">
                ✕ {selectedMes}
              </button>
            )}
          </div>
        ...
        <ResponsiveContainer width="100%" height={280}>
          ...
          >
            ...
            <XAxis
              ...
              tick={({ x, y, payload }) => (
                <text
                  ...
                  fontSize={11}
                  fontWeight={payload.value === selectedMes ? "bold" : "normal"}
                >
                  {payload.value}
                </text>
              )}
            ...
            <Bar
              yAxisId="left" dataKey="receita_novos" name="1º Fechamento" stackId="a"
              radius={[0, 0, 0, 0]} cursor="pointer"
            >
              {RECEITA_MES_RAW.map((d) => (
                <Cell
                  key={d.mes}
                  fill={d.mes === selectedMes ? "#60a5fa" : "#3b82f6"}
                  opacity={selectedMes && d.mes !== selectedMes ? 0.4 : 1}
                />
              ))}
            </Bar>
            <Bar
              yAxisId="left" dataKey="recompra" name="Recompra" stackId="a"
              radius={[4, 4, 0, 0]} cursor="pointer"
            >
              {RECEITA_MES_RAW.map((d) => (
                <Cell
                  key={d.mes}
                  fill={d.mes === selectedMes ? "#c084fc" : "#a855f7"}
                  opacity={selectedMes && d.mes !== selectedMes ? 0.4 : 1}
                />
              ))}
            </Bar>
        ...
        {selectedMes && mesSelecionado && (
          <div className="mt-4 bg-gray-800 border border-blue-800 rounded-xl p-4 grid grid-cols-2 sm:grid-cols-4 gap-4 animate-in fade-in">
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
          ...
          <p className="text-gray-600 text-xs mb-4">Passe o mouse ou clique para ver detalhes</p>
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              ...
              <Legend formatter={(v, e, i) => (
                <span style={{ color: activePie === i ? "#fff" : "#d1d5db", fontSize: 12, fontWeight: activePie === i ? "bold" : "normal" }}>{v}</span>
              )} iconSize={10} />
            </PieChart>
          </ResponsiveContainer>
          {activePie !== null && (
