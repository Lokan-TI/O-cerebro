...
// Tooltip customizado mês retenção
...
export default function TabGoogleRetencao() {
  ...
  // clientes que fizeram exatamente N recompras (para detalhe do gráfico de distribuição)
  const { CLIENTES_WON } = { CLIENTES_WON: [] }; // placeholder — não usaremos drill-down de clientes aqui
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
          <h2 className="text-white font-semibold mb-1 text-sm uppercase tracking-wider">
            Distribuição de Recompras entre Clientes WON
          </h2>
          <p className="text-gray-600 text-xs mb-4">Passe o mouse para detalhar</p>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={distribData}
              onMouseLeave={() => setActiveDistrib(null)}
            >
              ...
              <Bar
                ...
                radius={[4, 4, 0, 0]}
                cursor="pointer"
                onMouseEnter={(_, i) => setActiveDistrib(i)}
              >
                {distribData.map((_, i) => (
                  <Cell
                    key={i}
                    fill={BAR_COLORS[i % BAR_COLORS.length]}
                    opacity={activeDistrib === null || activeDistrib === i ? 1 : 0.35}
                  />
                ))}
              </Bar>
            </BarChart>
            ...
            >
              ...
              <Bar dataKey="receita_retida" radius={[4, 4, 0, 0]} name="Receita Retida" cursor="pointer">
                {RETIDO_POR_MES.map((d) => (
                  <Cell
                    key={d.mes}
                    fill={d.mes === selectedMes ? "#fcd34d" : "#f59e0b"}
      ...
      {/* Acumulado — interativo com dots clicáveis */}
      <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={RETIDO_POR_MES}>
            ...
            <Line type="monotone" dataKey="acumulado" stroke="#f59e0b" strokeWidth={2.5}
              dot={{ fill: "#f59e0b", r: 5, strokeWidth: 0, cursor: "pointer" }}
